...
class RunStageAndDepthComeFromTheBusinessContract(unittest.TestCase):
    ...
    def test_the_runner_takes_them_from_the_workbook_and_lets_the_cli_win(self):
        runner = self._runner()
        for fragment in ("contract_stage, contract_depth, workbook_search_controls = "
                         "_contract_run_settings(input_path)",
...
class LanguageCoverageWindowsCanBoundWorkingHours(unittest.TestCase):
    ...
    def test_a_shift_that_starts_inside_the_window_may_run_past_its_end(self):
        """Coverage windows constrain shift starts, not legal shift duration."""
        self.assertTrue(E.shift_within_language_window(
    ...
    def test_shift_duration_does_not_change_start_window_eligibility(self):
    ...
    def test_shift_start_at_window_end_is_outside(self):
        self.assertFalse(E.shift_within_language_window(
...
class DaySpecificLanguageCoverage(unittest.TestCase):
    """Coverage Days changes only the weekdays named by the input row."""

    def test_parser_creates_separate_weekday_rules_and_windows(self):
        import openpyxl

        wb = openpyxl.Workbook()
        ws = wb.active
        ws.title = "Language Setup"
        ws.append(["Language Setup"])
        ...
        ws.append(["International", "00:00", "20:00", "International", "Yes", "International", 1,
                   "Sun,Mon,Tue,Wed,Thu"])
        ws.append(["International", "00:00", "17:00", "International", "Yes", "International", 1, "Fri"])
        rules, capabilities, windows = E._parse_language_rules(
            wb, [SimpleNamespace(language="International")]
        )
        self.assertEqual(len(rules), 2)
        parsed = SimpleNamespace(language_rules=rules)
        self.assertEqual(len(E.language_rules_at(parsed, 9 * 60, day=0)), 1)
        self.assertEqual(len(E.language_rules_at(parsed, 18 * 60, day=0)), 1)
        self.assertEqual(len(E.language_rules_at(parsed, 16 * 60 + 45, day=5)), 1)
        self.assertEqual(len(E.language_rules_at(parsed, 17 * 60, day=5)), 0)
        self.assertEqual(len(E.language_rules_at(parsed, 9 * 60, day=6)), 0)
        self.assertEqual(windows["international@@5"], (0, 17 * 60, True))
        self.assertNotIn("international@@6", windows)

    def test_legacy_language_rows_still_apply_every_day(self):
        import openpyxl
        ...
        ws.append(["International", "00:00", "20:00", "International", "Yes", "International", 1])
        rules, _, _ = E._parse_language_rules(wb, [SimpleNamespace(language="International")])
        parsed = SimpleNamespace(language_rules=rules)
        for day in range(7):
            self.assertEqual(len(E.language_rules_at(parsed, 9 * 60, day=day)), 1)
