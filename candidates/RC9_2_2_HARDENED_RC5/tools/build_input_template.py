...
SEEDED_DEFAULTS = {"runstage": "Full Schedule", "rundepth": "Quick"}
...
def ensure_language_setup_controls(wb):
    """Add the day-scope field and enforced dropdown to an existing skill sheet."""
    ...
    days_col = next((c for label, c in headers.items()
                     if "coverage days" in label or "active days" in label or "days active" in label), None)
    if days_col is None:
        ...
        if days_col > 1:
            ...
            ws.cell(header, days_col).font = copy(ws.cell(header, days_col - 1).font)
            ws.cell(header, days_col).fill = copy(ws.cell(header, days_col - 1).fill)
            ws.cell(header, days_col).border = copy(ws.cell(header, days_col - 1).border)
            ws.cell(header, days_col).alignment = copy(ws.cell(header, days_col - 1).alignment)
    ws.column_dimensions[ws.cell(header, days_col).column_letter].width = 20
    dv = DataValidation(type="list", formula1=LANGUAGE_DAYS_CHOICES, allow_blank=True)
    dv.errorStyle = "stop"
    dv.showErrorMessage = True
