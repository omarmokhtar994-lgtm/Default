...
class InsufficientHeadcountProducesABestEffortScheduleNotNothing(unittest.TestCase):
    """A short roster is a business fact, not a broken workbook.

    NMG EN stopped at FAIL_PRE_SOLVER_CONTRACT because Sunday 02:00 has zero
    English-eligible associates against a minimum of 1. Nothing about the
    contract is wrong; the people do not exist. Refusing to produce a schedule
    helps nobody - the planner needs the best achievable schedule plus a clear
    statement of what cannot be covered.
    """

    def test_capacity_shortfalls_are_separated_from_contract_errors(self):
        failures = [
            {"code": "LANGUAGE_WINDOW_MAX_CAPACITY_BELOW_MINIMUM", "group": "English"},
            {"code": "AGGREGATE_TARGET_CAPACITY_SHORTAGE"},
            {"code": "ZERO_ACTIVE_INTERVAL_PROVABLY_IMPOSSIBLE"},
            {"code": "INVALID_TARGET_RATIO"},
            {"code": "UNKNOWN_FIXED_SHIFT"},
        ]
        errors, shortfalls = E.classify_preflight_failures(failures)
        self.assertEqual(
            [f["code"] for f in errors],
            ["ZERO_ACTIVE_INTERVAL_PROVABLY_IMPOSSIBLE", "INVALID_TARGET_RATIO",
             "UNKNOWN_FIXED_SHIFT"],
            "an unstaffable interval stays fatal alongside real contract errors")
        self.assertEqual(len(shortfalls), 2)

    def test_an_unstaffable_interval_is_never_downgraded(self):
        """The zero-active rule is hard on purpose.

        Relaxing a language identity - cover the interval with whoever is
        available - is a different decision from allowing an interval with
        nobody in it. The first is arguably a business choice; the second is
        the exact outcome the rule exists to prevent.
        """
        for code in ("ZERO_ACTIVE_INTERVAL_PROVABLY_IMPOSSIBLE",
                     "NEXT_SUNDAY_ZERO_ACTIVE_PROVABLY_IMPOSSIBLE"):
            with self.subTest(code=code):
                self.assertNotIn(code, E.CAPACITY_SHORTFALL_CODES)

    def test_best_effort_is_off_unless_the_business_turns_it_on(self):
        """Covering a language window with the wrong language is their call."""
        source = (ROOT / "engine" / "_tools" / "l632_universal_scheduler.py").read_text()
        self.assertTrue(
            '"Allow Best Effort Capacity Shortfall"], "No"' in source,
            "the default must preserve today's stop-hard behaviour")
        import dataclasses
        fields = {f.name: f for f in dataclasses.fields(E.ParsedInput)}
        self.assertIs(fields["best_effort_under_capacity_shortfall"].default, False)

    def test_a_stopped_run_still_explains_the_shortfall(self):
        """Stopping is fine; stopping without saying what to hire is not."""
        source = (ROOT / "engine" / "_tools" / "l632_universal_scheduler.py").read_text()
        self.assertTrue(
            "STOPPED_CAPACITY_SHORTFALL_BEST_EFFORT_DISABLED" in source)
        self.assertTrue(
            "This is a staffing shortfall, not a workbook error" in source,
            "the message must distinguish a short roster from a bad workbook")

    def test_a_broken_contract_is_never_downgraded(self):
        """Only capacity is forgiven. A bad workbook must still stop the run."""
        for code in ("INVALID_TARGET_RATIO", "INVALID_SHRINKAGE", "NO_ROSTER",
                     "UNKNOWN_BREAK_PATTERN", "CONTRADICTORY_EXACT_SCHEDULES_IN_NESTING_GROUP",
                     "ROSTER_SHAPE_MISMATCH", "NO_LEGAL_SHIFTS"):
            with self.subTest(code=code):
                self.assertNotIn(code, E.CAPACITY_SHORTFALL_CODES)

    def test_no_failures_means_no_shortfall(self):
        self.assertEqual(E.classify_preflight_failures([]), ([], []))

    def test_the_shortfall_summary_is_actionable(self):
        shortfalls = [
            {"code": "LANGUAGE_WINDOW_MAX_CAPACITY_BELOW_MINIMUM", "day": "Sun",
             "time": "02:00", "group": "English", "minimum": 2, "maximum_possible": 0},
            {"code": "LANGUAGE_WINDOW_MAX_CAPACITY_BELOW_MINIMUM", "day": "Sun",
             "time": "02:15", "group": "English", "minimum": 1, "maximum_possible": 0},
            {"code": "ZERO_ACTIVE_INTERVAL_PROVABLY_IMPOSSIBLE", "day": "Sun", "time": "02:00"},
        ]
        summary = E.summarize_capacity_shortfall(shortfalls)
        self.assertEqual(summary["affected_interval_count"], 3)
        self.assertEqual(summary["largest_headcount_gap"], 2)
        by_code = {row["code"]: row for row in summary["by_code"]}
        self.assertEqual(by_code["LANGUAGE_WINDOW_MAX_CAPACITY_BELOW_MINIMUM"]["groups"], ["English"])
        self.assertTrue(by_code["LANGUAGE_WINDOW_MAX_CAPACITY_BELOW_MINIMUM"]["examples"])

    def test_relaxations_clamp_only_the_proven_cells(self):
        """Downgrading a warning does not make a CP model satisfiable.

        `model.Add(count >= rule.minimum)` on a cell with zero eligible staff is
        infeasible however it is reported, which is why the first attempt went
        from FAIL_PRE_SOLVER_CONTRACT straight to FAIL_HARD_CONTRACT_INFEASIBLE.
        """
        shortfalls = [{"code": "LANGUAGE_WINDOW_MAX_CAPACITY_BELOW_MINIMUM", "day": "Sun",
                       "time": "02:00", "group": "English", "minimum": 1, "maximum_possible": 0}]
        relaxations = E.build_capacity_relaxations(shortfalls)
        parsed = SimpleNamespace(capacity_relaxations=relaxations)
        rule = SimpleNamespace(group="English", minimum=1)
        self.assertEqual(E.effective_language_minimum(parsed, rule, 0, 120), 0,
                         "the proven-impossible cell is clamped to what exists")
        self.assertEqual(E.effective_language_minimum(parsed, rule, 0, 135), 1,
                         "a neighbouring interval keeps the full contract minimum")
        self.assertEqual(E.effective_language_minimum(parsed, rule, 1, 120), 1,
                         "a different day keeps the full contract minimum")

    def test_a_relaxation_never_raises_a_minimum(self):
        relaxations = {(0, 120, "English"): 9}
        parsed = SimpleNamespace(capacity_relaxations=relaxations)
        rule = SimpleNamespace(group="English", minimum=2)
        self.assertEqual(E.effective_language_minimum(parsed, rule, 0, 120), 2)

    def test_without_relaxations_every_minimum_is_the_contract_minimum(self):
        parsed = SimpleNamespace(capacity_relaxations={})
        rule = SimpleNamespace(group="English", minimum=3)
        self.assertEqual(E.effective_language_minimum(parsed, rule, 0, 120), 3)

    def test_the_result_can_never_read_as_a_clean_pass(self):
        source = (ROOT / "engine" / "_tools" / "l632_universal_scheduler.py").read_text()
        self.assertTrue(
            'audit["status"] = f"BEST_EFFORT_CAPACITY_SHORTFALL__{audit[\'status\']}"' in source,
            "a best-effort run must be marked at the status level")
        self.assertTrue('"capacity_shortfall_status"' in source,
                        "the summary must carry the shortfall status")

    def test_best_effort_is_workbook_controlled(self):
        source = (ROOT / "engine" / "_tools" / "l632_universal_scheduler.py").read_text()
        self.assertTrue("Best Effort Under Capacity Shortfall" in source)
