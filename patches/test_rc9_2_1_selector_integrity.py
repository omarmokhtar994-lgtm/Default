...
class Stage1FundsDepthBeforeWidth(unittest.TestCase):
    """The slice must clear the point at which a solve produces nothing.
    ...
    the portfolio ran it anyway at a depth that produced nothing.

    The no-solution point itself was re-measured. The original probe
    (evidence/STAGE1_SLICE_DEPTH_PROBE.md) ran build_skeleton COLD, with no
    hint, and found AE AR B2B returning UNKNOWN at 45s and 150s. The Stage-1
    portfolio loop never solves cold - it always passes hint_skeleton - and
    warm on the same workbook, profile and seed
    (evidence/B3_STAGE1_PORTFOLIO_FLOOR.md) the cliff is between 15s and 30s:

        15s UNKNOWN | 30s 168/168 | 45s 168/168 | 240s 168/168

    So 45s is a successful attempt under the conditions the loop actually
    creates, and the floor is set from that curve rather than the cold one.
    """

    def test_the_minimum_slice_is_above_the_measured_no_solution_point(self):
        # 15s is where a WARM solve returns UNKNOWN - the conditions the
        # portfolio loop runs under. Below this the slice buys no skeleton.
        self.assertGreater(
            E.STAGE1_MIN_MEANINGFUL_SLICE_SEC, 15.0,
            "15s is the slice at which a warm-started AE AR B2B profile "
            "produced no skeleton at all")

    def test_the_minimum_slice_is_a_measured_point_not_a_round_guess(self):
        # Every value in the warm curve at or above this floor was measured
        # FEASIBLE at full quality, so the floor is a point on the curve.
        self.assertIn(E.STAGE1_MIN_MEANINGFUL_SLICE_SEC, {30.0, 45.0, 60.0, 120.0, 240.0})

    def test_the_floor_is_not_still_the_cold_measurement(self):
        # 240 was derived from a cold probe and applied to warm solves; that
        # mismatch is what made a 900s run attempt 0 of 15 profiles.
        self.assertLess(
            E.STAGE1_MIN_MEANINGFUL_SLICE_SEC, 240.0,
            "240s came from a cold-start curve the portfolio loop never runs")
    ...
    def test_the_real_regression_window_never_claims_the_whole_portfolio(self):
        """150s was the actual AE AR B2B Stage-1 window for 15 profiles.

        The point of this test is that a small window must not report that it
        can pay for a fifteen-profile portfolio. It must still report what it
        CAN pay for: with the warm floor a 150s window honestly funds two
        45-second profiles, and two is both truthful and far short of fifteen.
        """
        funded = E.stage1_fundable_profile_count(150)
        self.assertLess(funded, 15,
                        "a 150-second window cannot fund a fifteen-profile portfolio")
        self.assertLessEqual(
            funded * E.STAGE1_MIN_MEANINGFUL_SLICE_SEC,
            150 * E.STAGE1_SLICE_UTILIZATION + 1e-9,
            "what it claims to fund must fit inside the window")

    def test_a_window_the_old_floor_threw_away_now_funds_real_profiles(self):
        # The B-3 regression: a 171s window with a 240s floor funded nothing
        # and the time went to Stage 2 to polish a worse skeleton.
        self.assertGreaterEqual(E.stage1_fundable_profile_count(171), 1)

    def test_the_slice_never_drops_below_the_floor(self):
    ...
    def test_the_loop_uses_the_helper(self):
        ...
        self.assertTrue(
            ...
            "stage1_minimum_slice_sec)" in source,
            "the Stage-1 loop must take its slice from the guarded helper")

    def test_the_helper_still_guards_when_the_floor_is_workbook_stated(self):
        # The floor became reachable from the workbook; the guarantees the
        # helper makes must not depend on which value it is given.
        for floor in (30.0, 240.0, 600.0):
            with self.subTest(floor=floor):
                self.assertGreaterEqual(E.stage1_slice_seconds(10 ** 6, 40, floor), floor)
                self.assertLessEqual(E.stage1_slice_seconds(10 ** 6, 1, floor),
                                     E.STAGE1_MAX_SLICE_SEC)
                # Never more than the usable part of the window it was given.
                self.assertLessEqual(E.stage1_slice_seconds(100.0, 1, floor),
                                     max(floor, 100.0 * E.STAGE1_SLICE_UTILIZATION))

    def test_a_nonpositive_floor_is_rejected_rather_than_dividing_by_zero(self):
        for floor in (0.0, -1.0):
            with self.subTest(floor=floor):
                with self.assertRaises(ValueError):
                    E.stage1_fundable_profile_count(1000.0, floor)
