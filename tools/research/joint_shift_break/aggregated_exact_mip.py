...
BACK = sys.argv[7] if len(sys.argv) > 7 else "HIGHS"
m = pywraplp.Solver.CreateSolver(BACK)
assert m is not None, BACK
y = {}
for ci, members in enumerate(cls):
    ts = tours_for(members)
    for t in ts: y[(ci, t)] = m.IntVar(0, len(members), "")
...
for (d, s), vs in cnt.items():
    ...
    if not plist:
        ...
        continue
    nv = [m.IntVar(0, A, "") for _ in plist]
    m.Add(sum(nv) == total)
...
def cap_rows(raw, brk):
    # brk <= min(raw-1, max(1, floor(ratio*raw)), ABS); brk integer so floor is implied
    z = m.BoolVar("")
    m.Add(brk <= 1 + ABS * z); m.Add(brk <= ratio * raw + ABS * (1 - z))
    m.Add(brk <= ABS); m.Add(brk <= raw - 1)
...
for (d, i) in active:
    ...
    ht, hf = m.BoolVar(""), m.BoolVar("")
    m.Add(tot >= Tt * ht); m.Add(tot >= Tf * hf)
    if ALLHIT: m.Add(ht == 1)
    hits.append((ht, hf))
    for q in qs:
        m.Add(on_expr(q) >= 1)
        if brk_terms.get(q): cap_rows(raw_expr(q), brk_expr(q))
for q in range(672, 768):
    ...
    if brk_terms.get(q) or brk_terms.get(q - 672):
        cap_rows(sum(raw_terms[q]) + sum(raw_terms.get(q - 672, [])),
                 sum(brk_terms.get(q, [])) + sum(brk_terms.get(q - 672, [])))
...
def hint_from(skel_idx, selected, fix=False):
    ...
    for k, v in y.items():
        (m.Add(v == tc.get(k, 0)) if fix else (HINT_VARS.append(v), HINT_VALS.append(tc.get(k, 0))))
    for k, v in n.items():
        (m.Add(v == nc.get(k, 0)) if fix else (HINT_VARS.append(v), HINT_VALS.append(nc.get(k, 0))))
...
if MODE.startswith("planted"):
    ...
    h = json.load(open(MODE.split(":", 1)[1]))
    hint_from(h["selected_shift_index"], {(a, d): p for a, d, p in h["selected_pattern"]}, fix=MODE.startswith("jsonfix"))
if HINT_VARS: m.SetHint(HINT_VARS, HINT_VALS)
m.SetTimeLimit(int(T * 1000)); m.SetNumThreads(W)
m.EnableOutput() if "-v" in sys.argv else None
st = m.Solve()
class _SV:
    def Value(self, v): return int(round(v.solution_value()))
sv = _SV()
th = sum(sv.Value(h) for h, _ in hits); fh = sum(sv.Value(f) for _, f in hits)
print(f"  {BACK} status {st}: target {th}/{len(active)} floor {fh}  bound {m.Objective().BestBound()/1000:.2f} [{time.time()-t0:.0f}s]", flush=True)
