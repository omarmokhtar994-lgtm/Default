...
def roles(pareto):
 ...
 return out
def clean_book(src,dst,role,use,pareto):
...
def publish(root):
 root=root.resolve(); summary=read_csv(one(root,'*.l6_3_2_3_summary.csv')); pareto=json.loads(one(root,'*_PARETO_EXPORT_MANIFEST.json').read_text()); cid=Path(summary.get('output','')).name.split('_L6_3_2_3_',1)[0]; prod=root/'production'; prod.mkdir(exist_ok=True)
 before=clean_book(one(root,'*_BEST_BEFORE_BREAKS_SCHEDULE.xlsx'),prod/f'{cid}_L6_3_2_3_BEST_BEFORE_BREAKS_SCHEDULE.xlsx','BEST_BEFORE_BREAKS_SCHEDULE','REVIEW ONLY - strongest legal shift/OFF skeleton before breaks; not operational',pareto)
 final=clean_book(one_of(root,'*_BEST_FINAL_AFTER_BREAKS_SCHEDULE.xlsx','*_RECOMMENDED_FINAL_AFTER_BREAKS_SCHEDULE.xlsx'),prod/f'{cid}_L6_3_2_3_BEST_FINAL_AFTER_BREAKS_SCHEDULE.xlsx','BEST_FINAL_AFTER_BREAKS_SCHEDULE','PRODUCTION OUTPUT - use only when hard gates pass or a documented quality-debt approval exists',pareto)
 manifest={'release':RELEASE,'approval_status':APPROVAL,'generated_utc':datetime.now(timezone.utc).isoformat(),'case':cid,'solver':{'version':SOLVER,'week_boundary_patch_commit':COMMIT,'engine_sha256':ENGINE,'optimization_logic_changed':True,'change_scope':'RC9 preserves workbook target-first skeleton/final selection, protected target-champion break proof, joint/break refinement, and workbook-driven quality gates without client/case branching'},'two_artifact_contract':{'BEST_BEFORE_BREAKS_SCHEDULE':before,'BEST_FINAL_AFTER_BREAKS_SCHEDULE':final},'candidate_role_rows':roles(pareto),'source_status':summary.get('status','')}
 ...
 zp=prod/f'{cid}_{RELEASE}_PRODUCTION_ONLY.zip'
...
def main():
 ...
 if a.selfcheck:
  assert 'duplicate workbook suppressed' in roles({'exports':[{'role':'RECOMMENDED_FINAL','metrics':{'after_100':1}},{'role':'MAX_FLOOR_CANDIDATE','metrics':{'after_100':1}}]})[1][-1]; print('PRODUCTION OUTPUT POLISHER SELFCHECK: PASS'); return 0
